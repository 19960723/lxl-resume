...
PROFILES = {
    "frontend": {
        "out": OUT_FRONTEND,
        "title": "高级 Web 前端开发工程师 / Electron 前端开发工程师",
        "summary": [
            "7年以上 Web 前端开发经验，长期负责 Electron 客户端、后台管理、小程序、H5 嵌入页与可视化平台等多端业务交付。",
            "主攻复杂前端与桌面端应用，熟悉 Vue、TypeScript、Electron、Element UI、uni-app，具备从需求拆解、方案设计、模块开发到联调上线的完整交付经验。",
            "在多窗口通信、IM 通信、TRTC/WebRTC 音视频、文件同步、本地缓存、地图与大屏可视化等复杂业务中有持续实践，能把复杂交互拆成可维护的模块和公共能力。",
            "具备工程化和协作规范意识，能够沉淀通用组件、指令、进程通信机制和项目规范，支撑多人协作、长期迭代和老项目维护。",
        ],
        "skills": [
            "前端基础：JavaScript / TypeScript、Vue、React、Element UI、Ant Design Vue、vue-element-admin，熟悉组件化开发、状态管理、复杂表单、权限后台和多端适配。",
            "Electron 客户端：熟悉主进程/渲染进程通信、多窗口管理、窗口透明/点穿、截图、注册表读取、安装状态识别、本地文件与目录处理等桌面端能力。",
            "复杂业务集成：具备 TRTC/WebRTC、IM 通信、地图 API、ECharts 可视化、文件同步、本地缓存、微信小程序消息等能力接入与联调经验。",
            "工程化与质量：熟悉 Vite、Webpack、Gulp、ESLint、Prettier、Mock、Git/SVN，能够推进代码规范、公共逻辑抽象、构建配置和协作流程优化。",
    ...
    "fullstack": {
        ...
        "summary": [
            "7年以上 Web 应用开发经验，以复杂前端、Electron 客户端和业务系统交付为主，具备 Node 接口协作、部署配置和线上问题排查经验。",
            "熟悉 Vue、TypeScript、Electron、Node.js、Express、Koa2、Docker、Nginx，能够参与前端核心模块、基础接口、环境配置和发布链路的协同落地。",
        ...
        "skills": [
            "前端技术：JavaScript / TypeScript、Vue、React、Element UI、Ant Design Vue、vue-element-admin，熟悉组件化开发、复杂表单、权限后台和多端适配。",
            "Electron 客户端：熟悉主进程/渲染进程通信、多窗口管理、窗口透明/点穿、截图、注册表读取、安装状态识别、本地文件与目录处理等桌面端能力。",
...
WORK_EXPERIENCE = [
    (
        ...
        "厦门玄空科技有限公司",
        [
            "担任核心开发人员，负责 Electron 客户端核心能力、后台管理模块、音视频会议、IM 通信和文件同步等复杂业务迭代。",
            "参与需求评审、技术方案设计、复杂功能拆解、项目规范建设和跨端联调，推动核心模块稳定交付。",
        ],
    ),
    (
        "2021.04 - 2022.06",
        ...
        "厦门互啪智能科技有限公司",
        [
            "负责自研 PC 客户端办公应用、后台管理系统、H5 嵌入原生移动端等业务模块开发和版本迭代。",
            "参与 Electron 桌面端交互、富文本扩展、项目规范建设、公共逻辑封装和项目结构优化。",
        ],
    ),
    (
        "2018.08 - 2021.01",
        ...
        [
            ...
            "参与巨幕播放系统和可视化平台的功能重构、复杂交互扩展、数据可视化和多项目场景交付。",
        ],
    ),
    (
        "2018.01 - 2018.06",
...
def current_project_bullets(extra):
    return [
        "负责 Electron 客户端核心能力建设，落地多窗口通信、事件分发、窗口透明、点穿、画中画等桌面端交互能力。",
...
PROJECTS = [
    {
        ...
        "intro": "项目包含 Electron 客户端与后台管理两部分，覆盖桌面端多窗口交互、IM 通信、TRTC 音视频会议、文件同步、CG 审核、任务流程及人员/招聘/薪资/权限等后台模块。",
        "bullets": current_project_bullets,
    },
    {
        "date": "外接项目",
        "name": "澳门美容管理平台",
        "stack": "Vue2 / uni-app / JeecgBoot / Ant Design Vue / Vuex / Vue Router / Axios / ColorUI / 微信小程序",
        ...
        "bullets": [
            ...
            "独立完成门店后台端、会员小程序端、员工小程序端核心前端业务开发，覆盖会员、预约、销售、人事考勤、OA 协同、营销、仓存、报表和消息推送等模块。",
...
def add_body(doc, text, size=9.3, bold=False, color=COLOR_INK, after=2, before=0):
    p = doc.add_paragraph()
...
def add_bullet(doc, text, level=0):
    ...
    run = p.add_run(text)
    set_run_font(run, 9.05, False, COLOR_INK)
    return p
...
def add_role(doc, date, title, org, bullets):
    ...
    for text, bold, color in [
        ...
        run = p.add_run(text)
        set_run_font(run, 9.3, bold, color)
    for item in bullets:
        add_bullet(doc, item)
...
def add_project(doc, project, extra=None):
    ...
    set_run_font(run, 9.7, True, COLOR_INK)

    add_body(doc, project["intro"], size=9.05, after=1.2)
    add_body(doc, f"技术栈：{project['stack']}", size=9.05, bold=True, after=1.2)
...
def build(profile):
    ...
    set_run_font(run, 9.1, False, COLOR_MUTED)
    ...
    add_body(doc, BASE["education"], size=9.3, after=2)
